...
from graphs.models.netunits import *
from logzero import logger

class Net(nn.Module):
    def __init__(self, out_ch, d, embedding_dim, with_embedding, embedding_first_layer):
        ...
        self.down3 = Down(512, 512, d)
        self.down4 = Down(512, 512, d)
        self.upsamp = Upsamp()
        self.resize = Resize()
...
def create_model(inv_pl, inv_keylist, pl_keylist, args):
    if inv_pl == 1:
        keylist = inv_keylist
    else:
        keylist = pl_keylist
    return Net(len(keylist) + 1, args.dilation, args.embedding_dim, args.with_embedding, args.embedding_first_layer)
